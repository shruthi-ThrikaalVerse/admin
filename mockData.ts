...
// Realistic Indian names for employees
const INDIAN_NAMES = [
...
// Generate 20+ Employee records with full compensation metadata and sample documents
export const mockEmployees: EmployeeSummary[] = INDIAN_NAMES.map((name, index) => {
  ...
  return {
    ...
    password: `Pass@${(index + 1).toString().padStart(3, '0')}`, // Mock password
    ...
    documents,
    /**
     * Integrated mock salary structure for testing payroll runs
     */
    salaryStructure: {
    ...
    /**
     * Integrated mock bank details for disbursal simulation
     */
    bankDetails: {
      bankName: 'HDFC Bank',
...
// Mock Attendance Data - Diversified to include Late entries
export const mockAttendanceRecords: AttendanceRecord[] = mockEmployees.slice(0, 15).map((emp, idx) => ({
  employeeId: emp.employeeId,
  name: emp.fullName,
  date: '2024-05-15',
  checkIn: idx % 3 === 0 ? '09:45 AM' : '09:15 AM', // Some Late (9:45), Some Present (9:15)
  checkOut: '06:30 PM',
  totalHours: '09:15',
...
// Mock Leave Requests - Adjusted dates to overlap with 2024-05-15
