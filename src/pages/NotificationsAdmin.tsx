...
import { AdminNotification, AdminNotificationType, AdminNotificationPriority, AdminNotificationStatus } from '../types';
                ...
                <tr key={n.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-2">
                       <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${
                         n.type === 'global' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-blue-50 text-blue-600 border-blue-100'
                       }`}>
                  ...
                  <td className="px-8 py-6 text-center">
                     <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest border ${
                        n.priority === 'urgent' ? 'bg-rose-50 text-rose-600 border-rose-100' :
                  ...
                  <td className="px-8 py-6 text-right">
                    <div className="flex justify-end gap-2">
                      <button onClick={() => handleEdit(n)} className="p-2 text-slate-300 hover:text-indigo-600 hover:bg-white rounded-xl shadow-sm border border-transparent transition-all"><Icon name="Edit3" className="w-4 h-4" /></button>
                      <button onClick={() => deleteAdminNotification(n.id)} className="p-2 text-slate-300 hover:text-rose-500 hover:bg-white rounded-xl shadow-sm border border-transparent transition-all"><Icon name="Trash2" className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              )) : (
                    ...
                    {(['global', 'selected'] as const).map(type => (
                      <button
                        ...
                        className={`flex-1 py-3 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all ${
                          formData.type === type ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-white border-slate-100 text-slate-400 hover:bg-slace-50'
                        }`}
                      >
