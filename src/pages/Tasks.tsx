            ...
            <div key={team.id} className="bg-white border border-slate-100 rounded-[32px] p-8 hover:shadow-2xl hover:shadow-indigo-500/5 transition-all group flex flex-col">
              <div className="flex items-center justify-between mb-6">
                 ...
                 <div className="flex gap-1">
                    <button onClick={() => handleEditTeam(team)} className="p-2 text-slate-200 hover:text-indigo-600 transition-colors">
                        <Icon name="Edit3" className="w-5 h-5" />
                    </button>
                    <button onClick={() => deleteCustomTeam(team.id)} className="p-2 text-slate-200 hover:text-rose-500 transition-colors">
                ...
                {(['employee', 'team', 'department'] as const).map(type => (
                  <button
                    ...
                    className={`flex-1 py-3 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all ${
                      newTask.assigneeType === type ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-white border-slate-100 text-slate-400 hover:bg-slate-50'
                    }`}
                  >
                    {type}
                  </button>
                 ...
                 <div className="flex gap-2">
                    {(['low', 'medium', 'high'] as const).map(p => (
                      <button
                        ...
                        className={`flex-1 py-3 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all ${
                          newTask.priority === p ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-white border-slate-100 text-slate-400 hover:bg-slate-50'
                        }`}
                      >
                        {p}
                      </button>
