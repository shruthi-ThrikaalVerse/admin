...
import { PayslipData, EmployeeSummary, PayrollRun } from '../types';
...
const PayrollProcessing: React.FC = () => {
  ...
  const [runMonth, setRunMonth] = useState('May');
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group">
               ...
               <div className="mt-4 flex items-center justify-between">
                  ...
                  <button onClick={handleRunPayroll} disabled={isRunning} className="px-4 py-2 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-indigo-700 transition-all shadow-lg active:scale-95 disabled:opacity-50">
          ...
          <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden">
            <div className="p-8 border-b flex items-center justify-between">
              ...
              <button className="p-3 hover:bg-slate-50 rounded-2xl transition-colors text-slate-400"><Icon name="Filter" className="w-5 h-5" /></button>
            </div>
                        ...
                        <tr key={run.id} className="hover:bg-slate-50 transition-colors group">
                          ...
                          <td className="px-8 py-6 text-right">
                            <div className="flex justify-end gap-2">
                              <button onClick={() => handleDownloadRun(run)} className="p-2 text-slate-300 hover:text-indigo-600 hover:bg-white rounded-xl shadow-sm border border-transparent transition-all"><Icon name="Download" className="w-4 h-4" /></button>
                              <button onClick={() => handleViewRun(run)} className="p-2 text-slate-300 hover:text-indigo-600 hover:bg-white rounded-xl shadow-sm border border-transparent transition-all"><Icon name="Eye" className="w-4 h-4" /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
              ...
              <tbody className="divide-y divide-slate-50">
                {payslips.length > 0 ? payslips.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-8 py-6 text-right">
                      <button onClick={() => setSelectedPayslip(p)} className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all"><Icon name="FileText" className="w-5 h-5" /></button>
                    </td>
                  </tr>
                )) : (
                  <tr>
                    <td colSpan={6} className="py-20 text-center text-slate-400 font-black uppercase text-xs tracking-[0.2em]">Run payroll to generate May slips</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden animate-in slide-in-from-bottom-4">
           <div className="p-8 border-b bg-slate-50/50 flex items-center justify-between">
              ...
              <div className="relative group">
                 ...
                 <input type="text" placeholder="Lookup staff salary..." className="pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-2xl outline-none text-sm font-medium w-64 focus:ring-2 focus:ring-indigo-500 transition-all shadow-sm" />
              </div>
           </div>
           <div className="overflow-x-auto">
             <table className="w-full text-left">
                 ...
                 {employees.map(emp => {
                    ...
                    return (
                      <tr key={emp.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-8 py-6">
                           <div className="flex items-center gap-3">
                              <img src={emp.avatar} className="w-9 h-9 rounded-xl shadow-sm" alt="" />
                              <div>
                        ...
                        <td className="px-8 py-6 text-right">
                           <button onClick={() => setSelectedEmpForSalary(emp)} className="p-3 bg-white border border-slate-200 rounded-xl text-indigo-600 hover:bg-indigo-50 transition-all shadow-sm"><Icon name="Edit3" className="w-4 h-4" /></button>
                        </td>
                      </tr>
                    )
                 })}
         ...
         {selectedEmpForSalary && (
           <form onSubmit={handleUpdateSalaryAndBank} className="space-y-8 pb-4">
              <div className="flex items-center gap-4 p-5 bg-indigo-50 rounded-[28px] border border-indigo-100">
                 <img src={selectedEmpForSalary.avatar} className="w-14 h-14 rounded-2xl shadow-sm border-2 border-white" alt="" />
                 <div>
