...
const AttendanceMonitor: React.FC = () => {
  ...
  const dailyMasterFeed = useMemo(() => {
    ...
    return employees.map(emp => {
      ...
      const order = { late: 1, present: 2, 'on-leave': 3, absent: 4 };
      return (order[a.status] || 5) - (order[b.status] || 5);
    });
  }, [employees, attendance, leaves, selectedDate, lastRefresh]);

  // 2. Filter logic for the UI feed
  const filteredFeed = useMemo(() => {
    if (statusFilter === 'all') return dailyMasterFeed;
    return dailyMasterFeed.filter(f => f.status === statusFilter);
  }, [dailyMasterFeed, statusFilter]);

  // 3. Header Stats calculation
  const statsSummary = useMemo(() => {
    const total = employees.length;
    const presentCount = dailyMasterFeed.filter(a => a.status === 'present').length;
    const lateCount = dailyMasterFeed.filter(a => a.status === 'late').length;
    const onLeaveCount = dailyMasterFeed.filter(a => a.status === 'on-leave').length;
    const absentCount = dailyMasterFeed.filter(a => a.status === 'absent').length;
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select 
            ...
            className="px-3 py-2 bg-white border border-gray-300 rounded-lg text-xs font-bold uppercase tracking-wider text-gray-600 outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm"
          >
            {Array.from({ length: getDaysInMonth(selectedDate.getFullYear(), selectedDate.getMonth()) }, (_, i) => i + 1).map(d => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredFeed.length > 0 ? filteredFeed.map((record, i) => (
                ...
                <tr>
                  <td colSpan={6} className="py-24 text-center">
                    ...
                    <p className="text-gray-400 font-black uppercase text-xs tracking-widest">Zero {statusFilter !== 'all' ? statusFilter : ''} logs for this selection.</p>
                  </td>
                </tr>
              )}
            </tbody>
